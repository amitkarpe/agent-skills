---
name: web-html-page
description: fast low-token static html reports for codex tasks. use when the user says show html, web report, publish report, what changed, summarize task, show me in browser, or asks for a quick operational summary after a worker run. generate small self-contained offline html under /opt/agent-web with evidence in ~/.AGENTS-temp. do not create deep dashboards or rich diagrams.
---
...
- Use `scripts/publish_html.py` to copy output into `/opt/agent-web`, write evidence, validate no external links, and print the LAN URL.
